...
class Vehicle(models.Model):
    ...
    wash_order_ids = fields.One2many(
        "compose_auto_clean.carwash_order", "vehicle_id", string="Riwayat Cuci"
    )

    wash_count = fields.Integer(
        string="Total Cuci", compute="_compute_wash_stats", store=True
    )
    last_wash_date = fields.Date(
        string="Cuci Terakhir", compute="_compute_wash_stats", store=True
    )
        ...
        for rec in self:
            if rec.name and len(rec.name) < 3:
                raise ValidationError(
                    _("Plat nomor terlalu pendek! Masukkan format yang benar.")
                )

    @api.depends("wash_order_ids.payment_status", "wash_order_ids.order_date")
    def _compute_wash_stats(self):
        for rec in self:
            done_orders = rec.wash_order_ids.filtered(
                lambda x: x.payment_status == "lunas"
            )
            rec.wash_count = len(done_orders)
            rec.last_wash_date = (
                max(done_orders.mapped("order_date")) if done_orders else False
            )
